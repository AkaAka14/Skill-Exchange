...
router.post('/', async (req, res) => {
	try {
		const { skillName, skillType, category = '', level = 'intermediate' } = req.body;
		...
		try {
			const skillText = category
				? `${skillName.trim()} is a skill in the ${category} category. Someone with this skill can teach or practice ${skillName.trim()}.`
				: `This is a skill called "${skillName.trim()}". Someone who has this skill is able to teach or practice ${skillName.trim()}.`;
